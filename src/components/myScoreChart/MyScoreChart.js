...
const MyScoreChart = (props) => {
  ...
  const scoreOriginValue = formatScore(props.mainData.todayScore, props.mainData.score)
...
const CustomLabel = ({ viewBox, score = 0 }) => {
  ...
  return (
    <>
      <text x={cx - 15} y={cy - 5}>
        <tspan className="score">
          {score}
        </tspan>
      </text>
      <text x={cx - 20} y={cy + 15}>
        <tspan className="firstTxt">
           de votre 
        </tspan>
      </text>
      <text x={cx - 19} y={cy + 35}>
        <tspan className="secondTxt">
           objectif
        </tspan>
      </text>
    </>
    ...
    <ResponsiveContainer width="100%" height="100%">
        <PieChart width={160} height={160} >
            <Pie
                ...
                fill="#FFFFFF"
                >
                {data.map((name, value) => { // Display graph score with data
                    if (value === 1) {
